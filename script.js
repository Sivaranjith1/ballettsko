...
function sidenavContent(type) {
    let content = `
                <div id="sidenavBanner" class="sidenavContent">
                    ...
                    <h4 id="totalPris" class="center"></h4>
                </div>
...
function headerTotal() {
    totalPris.innerHTML = ''
    const total = handlekurvTotal()
    if (total === 0) return
    // const valuta = konverterPris(Number(total), prisIndex)

    totalPris.innerHTML = `Totalt: ${total} ${valuta[prisIndex].navn}`
}
...
//endre modalen til en Kjøp skjern
function buyModal() {
    buyCart()
...
function buyCart() {
    const content = modal1.querySelector(".modal-content");
    content.innerHTML = "<h1 class='center text-ellipsis weight'>Handlekurv</h1><h4 id='buyPris' class='center'></h4>"
    // buyTotal()
    buyRegnTotal()
    drawBuyCart(content)
}
...
function buyRegnTotal() {
    try {
        ...
        buyPris.innerHTML = `Totalt: ${total} ${valuta[prisIndex].navn}`
    } catch (err) {
        return
    }
